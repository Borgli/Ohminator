...
async def volume(message, client, db_guild, plugin):
    await message.delete()
    server = ohminator.utils.get_server(message.guild)
    if server.active_player is None or server.active_player.is_done():
        ...
        parameters = message.content.split()
        if len(parameters) < 2:
            current_volume = server.active_player.source.volume
        else:
            try:
        ...
        if len(parameters) < 2:
            ...
            await message.channel.send(f'{icon}: {message.author.name}, the volume was changed from '
                                       f'{int(server.active_player.source.volume*2*100.0)}% to '
                                       f'{int(current_volume*2*100.0)}%!')
            server.active_player.source.volume = current_volume
...
async def pause(message, client, db_guild, plugin):
    await message.delete()
    server = ohminator.utils.get_server(message.guild)
    if server.active_player is None:
        ...
        await message.channel.send(f'{message.author.name}: The player is not playing because it was '
...
async def resume(message, client, db_guild, plugin):
    await message.delete()
    server = ohminator.utils.get_server(message.guild)
    if server.active_player is None or server.active_player.is_done():
